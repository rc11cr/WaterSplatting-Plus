...
from water_splatting.losses_underwater import (
    MonocularDepthLoss, 
    OpacityLoss, 
    GrayWorldLoss, 
    SaturationLoss
...
class WaterSplattingModelConfig(ModelConfig):
    ...
    """Weight for monocular depth loss"""
    opacity_loss_mult: float = 0.05
    """Weight for opacity/background loss"""
    opacity_loss_thresh: float = 2.5 
    """Threshold (LAB distance) for judging if a color is similar to water background"""
    gray_world_loss_mult: float = 0.001
    """Weight for gray world loss"""
    saturation_loss_mult: float = 0.001
    """Weight for saturation loss"""
    saturation_loss_thresh: float = 0.7
    """Threshold for saturation penalty"""
    color_loss_start_step: int = 1000
    """Steps to start applying color restoration losses (Warm-up)"""
    enable_gradient_scaling: bool = True
    """Whether to boost gradients for distant points"""
    gradient_scaling_factor: float = 0.5 
    """Factor 'beta' for scaling: grad *= exp(beta * depth)"""
...
class WaterSplattingModel(Model):
    ...
    def populate_modules(self):
        ...
        self.loss_mono = MonocularDepthLoss()
        self.loss_op = OpacityLoss(threshold_sim=self.config.opacity_loss_thresh)
        self.loss_gw = GrayWorldLoss()
        self.loss_sat = SaturationLoss(threshold=self.config.saturation_loss_thresh)
    ...
    def after_train(self, step: int):
        ...
        with torch.no_grad():
            ...
            if self.config.enable_gradient_scaling:
                scale_factor = torch.exp(self.config.gradient_scaling_factor * self.depths.detach())                
                scale_factor = torch.clamp(scale_factor, max=100.0)
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        if "mono_depth" in batch and self.config.mono_depth_loss_mult > 0:
            mono_depth = self._downscale_if_required(batch["mono_depth"])     
            mono_depth = mono_depth.to(self.device)
            
            loss_dict["loss_mono"] = (
                self.loss_mono(outputs["depth"], mono_depth) 
                * self.config.mono_depth_loss_mult
            )

        if "background" in outputs and self.config.opacity_loss_mult > 0:
            loss_dict["loss_opacity"] = (
                self.loss_op(
                    outputs["pred_image"], 
                    outputs["background"], 
        ...
        if self.step > self.config.color_loss_start_step and "rgb_object" in outputs:
            if self.config.gray_world_loss_mult > 0:
                loss_dict["loss_gw"] = (
                    self.loss_gw(outputs["rgb_object"]) 
                    * self.config.gray_world_loss_mult
                )
            if self.config.saturation_loss_mult > 0:
                loss_dict["loss_sat"] = (
                    self.loss_sat(outputs["rgb_object"]) 
                    * self.config.saturation_loss_mult
                )
